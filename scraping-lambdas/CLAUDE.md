# Scraping-Lambdas Agent — In-Worktree Instructions
...
1. The bot received a `!claude <task>` message in the scraping-lambdas Discord channel.
2. It ran a Step 1 Claude process in the main repo (`scraping-lambdas/`) that: verified `main`, pulled, and ran `git worktree add worktrees/<branch> -b <branch> main` to create *your* worktree. Existing worktrees are left alone — cleanup only happens on explicit maintenance requests.
...
1. The user posted in a thread under the scraping-lambdas channel.
...
**If this is a maintenance request** (list/delete/prune worktrees, list/close PRs, free-form maintenance): you were spawned in `scraping-lambdas/` (main repo) with no thread and no new worktree. Just do the maintenance and write the summary.

### What you must do
- Write your final summary to `/tmp/claude-code-result-scraping-lambdas.md` — the bot reads that file and posts it back to Discord.
...
## Project Details
- **Project:** scraping-lambdas — a monorepo of AWS Lambda scrapers
- **Main repo (READ-ONLY, stays on `main`):** `/home/ubuntu/discord-claude-bot/scraping-lambdas/scraping-lambdas`
- **Worktree base:** `/home/ubuntu/discord-claude-bot/scraping-lambdas/worktrees/`
- **Convention:** worktree folder name = branch name
- **Subprojects (each with its own package.json):**
  - `morningstar-etfs/`
  - `stockanalysis-etfs/`
  - `stockanalysis-stocks/`
  - `stockanalysis-daily-movers/`
...
3. Do NOT run git commands in the main repo (`scraping-lambdas/`).
4. Never commit on `main`.

## Build Commands (per-subproject)
There is no top-level package.json. Run scripts inside the subproject folder you touched:
- `yarn compile` — tsc typecheck
- `yarn prettier-check` / `yarn prettier-fix`
- `yarn lint-fix` (if the subproject defines it)

## When finished with a coding task
1. Run quality checks in the affected subproject(s): `yarn compile && yarn prettier-check`
...
5. Write summary to `/tmp/claude-code-result-scraping-lambdas.md`. The `PR:` line MUST point to the current open PR (the one you just pushed to or created), never a previously merged one.
...
1. Round 1: you push commits → open PR #10 → user merges → PR #10 closes.
2. Round 2 (same thread, same branch, `claude -c` resumes): branch still exists locally, worktree is intact, but PR #10 is gone. You must create PR #11 for the new commits.
3. Round 3: same again — PR #12, etc.
