...
function clearResultFiles(): void {
  for (const path of [SCRAPING_LAMBDAS_WORKTREE_RESULT, SCRAPING_LAMBDAS_TASK_RESULT, SCRAPING_LAMBDAS_ROUTE_RESULT]) {
    try {
...
function buildRouterPrompt(userMessage: string): string {
  return `You are a strict request router for a Discord bot serving the scraping-lambdas project. You do NOT execute the request — you only classify it and write a JSON decision file.
...
2. Write EXACTLY ONE valid JSON object to ${SCRAPING_LAMBDAS_ROUTE_RESULT} — no markdown, no code fences, no commentary, no surrounding text. Just the JSON.
...
async function routeRequest(userMessage: string): Promise<RouteDecision | { command: "router_error"; error: string }> {
  try {
    await runClaude(buildRouterPrompt(userMessage), { cwd: SCRAPING_LAMBDAS_MAIN_REPO });
  } catch (err) {
    return { command: "router_error", error: formatError(err) };
  }
  const raw = readResultFile(SCRAPING_LAMBDAS_ROUTE_RESULT);
...
2. CREATE — For the task: "${taskDescription}"
   ...
   - Create a new worktree+branch:
     \`git worktree add ${SCRAPING_LAMBDAS_WORKTREE_BASE}/<branch-name> -b <branch-name> main\`
...
4. Write result to ${SCRAPING_LAMBDAS_WORKTREE_RESULT}:
...
- Do NOT run git commands in ${SCRAPING_LAMBDAS_MAIN_REPO} (the main repo).

When completely finished with the task:
1. Run quality checks inside the relevant subproject folder: \`yarn compile && yarn prettier-check\` (and \`yarn lint-fix\` if the subproject has that script). Only run against the subproject(s) you actually touched.
2. Commit all changes
3. Push: git push -u origin ${branchName}
4. Create a PR if none exists: gh pr create --base main --head ${branchName} --title "..." --body "..."
5. Write your response to ${SCRAPING_LAMBDAS_TASK_RESULT}. **Two formats depending on what you did:**
   ...
   **A) If you made code changes (commits, files modified, PR created)** — write a STRICT 2-4 line summary, exactly these lines, nothing else:
   Line 1: one sentence describing what you changed (e.g., "Added retry logic to stockanalysis-etfs fetcher.")
...
If the task is ambiguous or needs clarification, do NOT make up requirements — write a single-line question to ${SCRAPING_LAMBDAS_TASK_RESULT} instead of committing code.`;
}
...
async function runGit(args: string[]): Promise<{ stdout: string; stderr: string }> {
  return execFileAsync("git", args, { cwd: SCRAPING_LAMBDAS_MAIN_REPO, maxBuffer: 1024 * 1024 });
}

async function runGh(args: string[]): Promise<{ stdout: string; stderr: string }> {
  return execFileAsync("gh", args, { cwd: SCRAPING_LAMBDAS_MAIN_REPO, maxBuffer: 1024 * 1024 });
}
...
async function handleNewTask(message: Message, taskDescription: string): Promise<void> {
  clearResultFiles();
  await message.reply(`Starting new scraping-lambdas task...\n**Task:** ${taskDescription}\n\n**Step 1/2:** Managing worktrees...`);
  ...
  const worktreeInfo = readResultFile(SCRAPING_LAMBDAS_WORKTREE_RESULT);
  ...
  try {
    thread = await channel.threads.create({
      ...
      reason: `Scraping-Lambdas task: ${taskDescription.slice(0, 100)}`,
    });
  } catch (err) {
  ...
  const taskResult = readResultFile(SCRAPING_LAMBDAS_TASK_RESULT);
