...
function clearResultFiles(): void {
  for (const path of [INSIGHTS_UI_WORKTREE_RESULT, INSIGHTS_UI_TASK_RESULT, INSIGHTS_UI_ROUTE_RESULT]) {
    try {
...
function buildRouterPrompt(userMessage: string): string {
  return `You are a strict request router for a Discord bot serving the insights-ui (KoalaGains) project. You do NOT execute the request — you only classify it and write a JSON decision file.
...
2. Write EXACTLY ONE valid JSON object to ${INSIGHTS_UI_ROUTE_RESULT} — no markdown, no code fences, no commentary, no surrounding text. Just the JSON.
...
async function routeRequest(userMessage: string): Promise<RouteDecision | { command: "router_error"; error: string }> {
  try {
    await runClaude(buildRouterPrompt(userMessage), { cwd: INSIGHTS_UI_MAIN_REPO });
  } catch (err) {
    return { command: "router_error", error: formatError(err) };
  }
  const raw = readResultFile(INSIGHTS_UI_ROUTE_RESULT);
...
4. Write result to ${INSIGHTS_UI_WORKTREE_RESULT}:
...
1. Run quality checks: yarn lint && yarn prettier-check
2. Commit all changes
3. Push: git push -u origin ${branchName}
4. Create a PR if none exists: gh pr create --base main --head ${branchName} --title "..." --body "..."
5. Write your response to ${INSIGHTS_UI_TASK_RESULT}. **Two formats depending on what you did:**
   ...
   **A) If you made code changes (commits, files modified, PR created)** — write a STRICT 2-4 line summary, exactly these lines, nothing else:
   Line 1: one sentence describing what you changed (e.g., "Reduced ETF listing page size from 100 to 25.")
...
If the task is ambiguous or needs clarification, do NOT make up requirements — write a single-line question to ${INSIGHTS_UI_TASK_RESULT} instead of committing code.`;
}
...
Address the new message in the context of the ongoing task. If it's a clarification, update the plan. If it's a follow-up change, make it. If you make code changes, run quality checks (yarn lint && yarn prettier-check), commit, push to origin ${branchName}, and update/create the PR.
...
async function runGit(args: string[]): Promise<{ stdout: string; stderr: string }> {
  return execFileAsync("git", args, { cwd: INSIGHTS_UI_MAIN_REPO, maxBuffer: 1024 * 1024 });
}

async function runGh(args: string[]): Promise<{ stdout: string; stderr: string }> {
  return execFileAsync("gh", args, { cwd: INSIGHTS_UI_MAIN_REPO, maxBuffer: 1024 * 1024 });
}
...
async function handleMaintenance(message: Message, taskDescription: string): Promise<void> {
  ...
  try {
    await runClaude(buildMaintenancePrompt(taskDescription), { cwd: INSIGHTS_UI_MAIN_REPO });
  } catch (err) {
...
async function handleNewTask(message: Message, taskDescription: string): Promise<void> {
  clearResultFiles();
  await message.reply(`Starting new insights-ui task...\n**Task:** ${taskDescription}\n\n**Step 1/2:** Managing worktrees...`);
  ...
  try {
    thread = await channel.threads.create({
      ...
      reason: `Insights-UI task: ${taskDescription.slice(0, 100)}`,
    });
  } catch (err) {
