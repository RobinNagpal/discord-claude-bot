# Discord-Claude-Bot Agent — In-Worktree Instructions
...
1. The bot received a `!claude <task>` message in the discord-claude-bot Discord channel.
2. It ran a Step 1 Claude process in the main repo clone (`discord-bot/discord-claude-bot/`) that: verified `main`, pulled, and ran `git worktree add worktrees/<branch> -b <branch> main` to create *your* worktree. Existing worktrees are left alone — cleanup only happens on explicit maintenance requests.
...
1. The user posted in a thread under the discord-claude-bot channel.
...
### What you must do
- Write your final summary to `/tmp/claude-code-result-discord-bot.md` — the bot reads that file and posts it back to Discord.
...
## Project Details
- **Project:** discord-claude-bot — a TypeScript Discord bot that bridges messages to Claude Code CLI
- **Runtime:** Node.js 20+, TypeScript (strict mode, ESM), discord.js
- **Main repo clone (READ-ONLY, stays on `main`):** `/home/ubuntu/discord-claude-bot/discord-bot/discord-claude-bot`
- **Worktree base:** `/home/ubuntu/discord-claude-bot/discord-bot/worktrees/`
...
3. Do NOT run git commands in the main repo clone (`discord-bot/discord-claude-bot/`).
...
5. Write summary to `/tmp/claude-code-result-discord-bot.md`. The `PR:` line MUST point to the current open PR (the one you just pushed to or created), never a previously merged one.
...
- `npm run prettier` / `npm run prettier:fix` — Prettier (printWidth: 160)
- `npm run build` — compile to `dist/`

## Key References
- Repo root `CLAUDE.md` for full development guidelines, project structure, and architecture
